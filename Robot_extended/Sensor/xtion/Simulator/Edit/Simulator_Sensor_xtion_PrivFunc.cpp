...
bool DECOFUNC(generateSourceData)(void * paramsPtr, void * varsPtr, void * outputData, QList<int> & outputPortIndex, QTime & timeStamp)
{
	...
	*/
    ...
    int ts = vars->textStream.readLine().toInt();
    outputdata->qtimestamp = QTime::fromMSecsSinceStartOfDay(ts);
    timeStamp = outputdata->qtimestamp;
//    qDebug() << ts << vars->frameNum;
    ...
    std::fclose(jointF);
    if (!outputdata->cvColorImg.data || !outputdata->cvDepthImg.data) {
//        qDebug() << colorFilename << depthFilename;
//        qDebug() << "Image error!!!" << !outputdata->cvColorImg.data << !outputdata->cvDepthImg.data;
        outputdata->cvColorImg = cv::Mat(480, 640,CV_8UC3);
